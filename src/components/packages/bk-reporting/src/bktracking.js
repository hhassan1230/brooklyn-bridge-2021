...
// Utilities
var anxDebug = function (msg) {};
if (typeof (window.anxDebug) === 'function') {
    anxDebug = window.anxDebug;
}
...
var JSUtil = {
    ...
    randomInt: function (max) {
        var n;
        if (JSUtil.isNotNull(window.crypto) && JSUtil.isNotNull(window.crypto.getRandomValues) && JSUtil.isNotNull(
            window.Uint32Array)) {
...
var _Anemone = {
    ...
    setCookies: function () {
        ...
        CookieUtil.setPermanentCookie(JSUtil.defaultVal(this.config.cookieName, 'anx'),
            c,
            this.getDomain(),
            false,
            this.cookieExpirationSecs);
    ...
    updateSession: function (isClientEvent) {
        ...
        if (!this.isNewSession) {
            if (JSUtil.defaultVal(this.sessionCookie.s, '-') === '-' || this.now - this.lastEvent > JSUtil.defaultVal(
                this.config.maxSession,
                30) * 60000) {
    ...
    getReferrer: function () {
        ...
        var m = JSUtil.defaultVal(WebUtil.getParamValue(JSUtil.defaultVal(this.config.mediumParam, 'anxrm')),
            defaultMedium);
